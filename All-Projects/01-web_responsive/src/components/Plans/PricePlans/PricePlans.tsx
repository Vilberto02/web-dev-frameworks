...
import { PricesPlansProps } from "./PricePlanes.type";
      ...
      {dataPricePlans.map(
        ({ id, name, description, prices, features, primary }) => (
          <MotionTransition key={id}>
            <div className="p-4 shadow-lg rounded-md shadow-pink-100">
              <h4 className="mb-5 text-2xl font-bold text-center">{name}</h4>
              <p className="mb-5 text-sm text-center">{description}</p>
              <div className="flex items-end justify-center">
                <span className="text-4xl font-black text-pink-500">
                  {isMonthly ? prices[0].price : prices[1].price}
                </span>
                <span className="ml-3 text-sm font-extrabold text-slate-400">
              ...
              <div className="my-6 text-center">
                <button
                  className={`font-medium px-8 py-3 rounded-xl 
                            ${
                              primary
